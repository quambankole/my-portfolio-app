...
export default function Header() {
    ...
    return (
    <header className={`fixed top-2.5 left-[50vw] px-10 -translate-x-1/2 w-[80%] border-round border-solid z-50 bg-white/20 py-2 mt-0 shadow-sm backdrop-blur-sm shadow-lg backdrop-saturate-150 transition-colors duration-300 rounded-4xl ${isDarkSection ? 'text-white' : 'text-black'}`}>
    <div className="p-3 flex justify-between items-center max-w-7xl mx-auto">
                ...
                <nav className={`${styles.navbar} ${navOpen ? styles.open : ''}`}>
                    <div className="flex space-x-2 text-2xl py-1 px-0 rounded-[0.25rem]">
                        <ul className={`flex lg:flex text-base lg:text-lg sm:text-sm/6 gap-5 lg:gap-12`}>
